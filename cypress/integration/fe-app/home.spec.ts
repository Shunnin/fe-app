describe('visit the home page', () => {
  ...
  it('display the header and search box by default', () => {
    ...
    cy.get('div.weather-card').should('have.length', 0);
    cy.get('div.weather-section__forecast').should('have.length', 0);
  });
  ...
  context('with search valid location', () => {
    ...
    it('can hover and click on the suggestion item', () => {
      ...
      cy.get('div.search-box__suggestions').should('have.length', 0);
      cy.get('div.search-box__suggestions-item').should('have.length', 0);
